...
{
    public class MenuToggle : UiElement
    {
        ...
        public MenuToggle(string text, string tooltip, Action<bool> onToggle, Transform parent, bool defaultValue = false) : this(text, tooltip, onToggle, parent, defaultValue, null) { }
        public MenuToggle(string text, string tooltip, Action<bool> onToggle, Transform parent, bool defaultValue = false, Sprite iconOn = null, Sprite iconOff = null) : base(QuickMenuEx.TogglePrefab, parent, $"Button_Toggle{text}")
        {
        ...
        public MenuToggle(Transform transform) : base(transform)
        {
            FindToggleIcon();
            _toggleComponent = GameObject.GetComponent<Toggle>();
        }
        ...
        private static readonly Dictionary<string, Type> DeobfuscatedTypes = new();
        ...
        private static void TryCacheDeobfuscatedType(Type type)
        {
            try
            {
                ...
                foreach (var att in type.CustomAttributes)
                {
                    ...
                    {
                        var obfuscatedName = att.ConstructorArguments[0].Value.ToString();
        ...
        private static Type GetUnhollowedType(Il2CppSystem.Type cppType)
        {
            if (DeobfuscatedTypes.Count == 0)
            {
