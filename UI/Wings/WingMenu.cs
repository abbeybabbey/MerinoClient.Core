...
namespace MerinoClient.Core.UI.Wings
{
    ...
    public class WingMenu : UiElement
    {
        ...
        public WingMenu(string text, bool left = true) : base(WingMenuPrefab, (left ? QuickMenuEx.LeftWing : QuickMenuEx.RightWing).field_Public_RectTransform_0, text, false)
        {
        ...
        public WingButton AddButton(string text, string tooltip, Action onClick, Sprite sprite = null, bool arrow = true, bool background = true, bool separator = false)
        {
            if (!GameObject)
                throw new NullReferenceException("This wing menu has been destroyed.");

            return new WingButton(text, tooltip, onClick, Container, sprite, arrow, background, separator);
        }

        public WingToggle AddToggle(string text, string tooltip, Action<bool> onToggle, bool defaultValue = false)
        {
            if (!GameObject)
                throw new NullReferenceException("This wing menu has been destroyed.");
        ...
        public WingMenu AddSubMenu(string text, string tooltip)
        {
            ...
            var menu = new WingMenu(text, _wing._wingType == WingType.Left);
